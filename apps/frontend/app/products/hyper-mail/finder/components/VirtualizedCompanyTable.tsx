...
const LogoCell = React.memo(({ logoUrl, companyName }: { logoUrl: string | null; companyName: string }) => (
  <Image src={logoUrl || "/images/company-fallback.png"} alt={companyName} width={36} height={36} unoptimized className="h-9 w-9 object-contain rounded-sm" />
));
LogoCell.displayName = "LogoCell";

const CompanyNameCell = React.memo(({ name }: { name: string }) => <span className="font-medium text-foreground truncate block">{name || "Unknown"}</span>);
CompanyNameCell.displayName = "CompanyNameCell";

const DomainCell = React.memo(({ domain }: { domain: string | null }) => {
...
DomainCell.displayName = "DomainCell";

const StatusCell = React.memo(({ verified }: { verified: boolean }) => (
  <Badge variant={verified ? "default" : "secondary"} className="text-xs px-2 py-0.5">
    {verified ? "Verified" : "Unverified"}
  </Badge>
));
StatusCell.displayName = "StatusCell";

const ConfidenceCell = React.memo(({ confidence }: { confidence: number | undefined }) => (
  <span className="text-xs text-muted-foreground tabular-nums">{typeof confidence === "number" ? `${confidence}%` : "—"}</span>
));
ConfidenceCell.displayName = "ConfidenceCell";

const SourceCell = React.memo(({ source }: { source: string | undefined }) => <span className="text-xs text-muted-foreground uppercase">{source || "—"}</span>);
SourceCell.displayName = "SourceCell";

const LinkedinCell = React.memo(({ linkedinUrl }: { linkedinUrl: string | null }) => {
  if (!linkedinUrl) {
    return (
      <Link href="#" target="_blank" rel="noreferrer" className="inline-flex items-center justify-center h-10 w-10">
        <Link2Off className="text-accent-foreground h-4 w-4" />
  ...
  return (
    <Link href={linkedinUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center h-10 w-10">
      <Image src={LinkedinLogo} alt="LinkedIn" width={24} height={24} className="h-6 w-6 hover:opacity-80 rounded-sm" />
    </Link>
  );
});
...
const columns: ColumnDef<CompanyResult>[] = [
  {
    accessorKey: "logo_url",
    header: "",
    cell: ({ row }) => <LogoCell logoUrl={row.original.logo_url} companyName={row.original.company_name} />,
    size: 48,
  },
  {
    accessorKey: "company_name",
    header: "Company",
    cell: ({ row }) => <CompanyNameCell name={row.original.company_name} />,
    size: 180,
  },
  {
    accessorKey: "domain",
  ...
  },
  {
    accessorKey: "verified",
    header: "Status",
    cell: ({ row }) => <StatusCell verified={row.original.verified} />,
    size: 90,
  },
  {
    accessorKey: "confidence",
    header: "confidence",
    cell: ({ row }) => <ConfidenceCell confidence={row.original.confidence} />,
    size: 60,
  },
  {
    accessorKey: "source",
  ...
  },
  {
    accessorKey: "linkedin_url",
    header: "Linkedin",
    cell: ({ row }) => <LinkedinCell linkedinUrl={row.original.linkedin_url} />,
    size: 40,
  },
];

// Memoized table row component
const VirtualTableRow = React.memo(({ row, index, onRowClick }: { row: Row<CompanyResult>; index: number; onRowClick: (company: CompanyResult) => void }) => {
  ...
  const handleCellClick = React.useCallback((e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest("a")) {
      e.stopPropagation();
    }
  }, []);
  ...
  return (
    <TableRow data-index={index} style={{ height: ROW_HEIGHT }} className={`${isEven ? "bg-muted/20" : ""} border-b transition-colors hover:bg-muted/50 cursor-pointer`} onClick={handleClick}>
      {row.getVisibleCells().map((cell) => (
        <TableCell key={cell.id} className="py-1.5 truncate" style={{ width: cell.column.getSize() }} onClick={handleCellClick}>
          {flexRender(cell.column.columnDef.cell, cell.getContext())}
        </TableCell>
      ))}
    </TableRow>
...
function VirtualizedCompanyTable({ data, hasMore, isFetchingNextPage, fetchNextPage, total, sortOrder = "asc", onSortToggle }: VirtualizedCompanyTableProps) {
  ...
  // Debounced infinite scroll with transition
  React.useEffect(() => {
    if (!virtualItems.length || !hasMore || isFetchingNextPage) return;
  ...
  // Memoized handlers
  const handleRowClick = React.useCallback((company: CompanyResult) => {
  ...
  const SortIcon = React.useMemo(() => {
    return sortOrder === "asc" ? ArrowUp : sortOrder === "desc" ? ArrowDown : ArrowUpDown;
  }, [sortOrder]);
  ...
  return (
    <div className="w-full h-full flex flex-col font-dmsans">
      ...
      {/* Table Container */}
      <div className="rounded-md border overflow-hidden flex-1 flex flex-col min-h-0">
        <div ref={tableContainerRef} className={`h-full overflow-auto relative scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent ${isPending ? "opacity-70" : ""}`}>
          <table className="w-full caption-bottom text-sm table-fixed">
            <TableHeader className="sticky top-0 z-20">
              {table.getHeaderGroups().map((headerGroup) => (
                <TableRow key={headerGroup.id} className="hover:bg-transparent border-b">
            ...
            <TableBody>
              {paddingTop > 0 && (
                <tr>
                  <td colSpan={memoizedColumns.length} style={{ height: paddingTop }} />
                </tr>
              )}

              {virtualItems.map((virtualRow) => {
                const row = rows[virtualRow.index] as Row<CompanyResult>;
                return <VirtualTableRow key={row.id} row={row} index={virtualRow.index} onRowClick={handleRowClick} />;
              })}

              {paddingBottom > 0 && (
                <tr>
                  <td colSpan={memoizedColumns.length} style={{ height: paddingBottom }} />
                </tr>
              )}
            </TableBody>
          </table>
      ...
      <CompanyDetailModal company={selectedCompany} open={selectedCompany !== null} onClose={handleModalClose} />
    </div>
  );
}
