...
class BrowserService {
  ...
  async getPage(options: ScrapeOptions = {}): Promise<Page> {
    ...
    const ctx = await browserInstance.newContext(await this.getContextOptions(proxyUrl, profile));
  ...
  private async getContextOptions(proxyUrl: string, profile: FingerprintProfile) {
  ...
  private async applyAdvancedFingerprint(page: Page, profile: FingerprintProfile) {
    // Block tracking resources to reduce noise
    await page.route("**/*", (route) => {
      const url = route.request().url();
      if (/google-analytics|facebook\.com\/tr|doubleclick|adservice|twitter\.com/i.test(url) || ["image", "font", "media", "stylesheet"].includes(route.request().resourceType())) {
        route.abort();
      } else {
        route.continue();
      }
    ...
    await page.addInitScript((fp: FingerprintProfile) => {
      ...
      (navigator as any).permissions.query = (parameters: any) => (parameters.name === "notifications" ? Promise.resolve({ state: Notification.permission }) : originalQuery(parameters));
  ...
  private async humanizeBehavior(page: Page) {
    await page.mouse.move(400 + Math.random() * 800, 200 + Math.random() * 400, { steps: 15 });
